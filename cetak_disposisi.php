    ...
    if(empty($_SESSION['admin'])){
        ...
        <!-- Container START -->
            ...
            <div id="lead">
                <p>Kepala Dinas</p>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT kepaladinas, nip FROM tbl_instansi");
                list($kepaladinas,$nip) = mysqli_fetch_array($query);
                if(!empty($kepaladinas)){
                    echo '<p class="lead">'.$kepaladinas.'</p>';
                } else {
                    echo '<p class="lead">Pak Yanyan</p>';
                }
                if(!empty($nip)){
                    echo '<p>NIP. '.$nip.'</p>';
                } else {
